...
export class ContextManager {
  ...
  public async createContext(name: string): Promise<void> {
    ...
    if (existsSync(this.settingsPath)) {
      ...
      console.log(`Context "${colors.bold(colors.green(name))}" created from current settings`);
      return;
    }
  ...
  public async renameContext(oldName: string, newName: string): Promise<void> {
    ...
    console.log(`Context "${oldName}" renamed to "${colors.bold(colors.green(newName))}"`);
  }
  ...
  public async importContextFromString(name: string, input: string): Promise<void> {
    this.validateContextName(name);
    const contexts = await this.listContexts();
    if (contexts.includes(name)) {
      throw new Error(`error: context "${name}" already exists`);
  ...
  public async interactiveRename(): Promise<void> {
    const { selectContext, promptInput } = await import("../utils/interactive.js");
    const contexts = await this.listContexts();
    if (contexts.length === 0) {
      console.log("No contexts found");
      return;
  ...
  public async mergeFrom(target: string, source: string, mergeFullFlag: boolean): Promise<void> {
    if (mergeFullFlag) {
      await this.mergeFromFull(target, source);
      return;
    }
    const { targetPath, targetJson, sourceJson } = await this.getMergePayload(target, source);
  ...
  public async unmergeFrom(target: string, source: string, mergeFullFlag: boolean): Promise<void> {
    ...
    const { targetPath, targetJson, contextName } = await this.getUnmergePayload(target, source);
  ...
  private async getMergePayload(target: string, source: string): Promise<{ targetPath: string; targetJson: Record<string, unknown>; sourceJson: Record<string, unknown> }> {
  ...
  private async getUnmergePayload(target: string, source: string): Promise<{ targetPath: string; targetJson: Record<string, unknown>; contextName: string }> {
    const targetPath = await this.resolveTargetPath(target);
    const targetJson = await readJson<Record<string, unknown>>(targetPath);
    const contextName = target === "current" ? (await this.getCurrentContext()) ?? "current" : target;
  ...
  private async resolveSourceJson(source: string): Promise<Record<string, unknown>> {
    if (source === "user") {
      const homeSettings = path.join(path.dirname(path.dirname(this.contextsDir)), "settings.json");
      if (!existsSync(homeSettings)) {
        throw new Error(`error: user settings file not found at ${homeSettings}`);
      }
  ...
  private async appendHistory(target: string, entry: ReturnType<typeof mergePermissions>): Promise<void> {
    const contextName = target === "current" ? (await this.getCurrentContext()) ?? "current" : target;
